    ...
    if battle.state /= Rolling || battle.remainingRerolls <= 0 then
        ...
        let
            ...
            updatedBattle =
                ...
                , state = if battle.remainingRerolls <= 1 then Selecting else Rolling
                }
        in
        (updatedBattle, newSeed)
            ...
            if not (Score.isScoreAvailable scoreType battle.scoreHistory) then
                ...
                let
                    ...
                    updatedBattle =
                        ...
                        , log = updatedLog
                        , state = if newEnemyHP <= 0 then BattleOver else EnemyTurn
                        }
                in
                updatedBattle
    ...
    if battle.state /= EnemyTurn then
        ...
        let
            ...
            updatedBattle =
                ...
                , log = updatedLog
                , state = if newPlayerHP <= 0 then BattleOver else Rolling
                }
        in
        updatedBattle
...
scoreTypeToString scoreType =
    case scoreType of
        Models.Types.Aces -> "エース"
        Models.Types.Twos -> "ツー"
        Models.Types.Threes -> "スリー"
        Models.Types.Fours -> "フォー"
        Models.Types.Fives -> "ファイブ"
        Models.Types.Sixes -> "シックス"
        Models.Types.Choice -> "チョイス"
        Models.Types.FourOfKind -> "フォーカインド"
        Models.Types.FullHouse -> "フルハウス"
        Models.Types.SmallStraight -> "Sストレート"
        Models.Types.LargeStraight -> "Lストレート"
        Models.Types.Yacht -> "ヨット"
        Models.Types.Special name -> name