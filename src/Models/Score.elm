...
isScoreAvailable scoreType history =
    case scoreType of
        ...
        FourOfKind ->
            not history.fourOfKind

        FullHouse ->
            not history.fullHouse

        SmallStraight ->
            not history.smallStraight

        LargeStraight ->
            not history.largeStraight

        Yacht ->
            not history.yacht

        Special id ->
            not (List.member id history.specialScores)
...
markScoreUsed scoreType history =
    case scoreType of
        ...
        FourOfKind ->
            { history | fourOfKind = True }

        FullHouse ->
            { history | fullHouse = True }

        SmallStraight ->
            { history | smallStraight = True }

        LargeStraight ->
            { history | largeStraight = True }

        Yacht ->
            { history | yacht = True }

        Special id ->
            { history | specialScores = id :: history.specialScores }
...
calculateFullHouse dice =
    let
        ...
        has3OfKind =
            List.any (\( _, count ) -> count == 3) valueCounts

        has2OfKind =
            List.any (\( _, count ) -> count == 2) valueCounts
    in
...
calculateSmallStraight dice =
    ...
    in
    if
        hasSubsequence uniqueValues [ 1, 2, 3, 4 ]
            || hasSubsequence uniqueValues [ 2, 3, 4, 5 ]
            || hasSubsequence uniqueValues [ 3, 4, 5, 6 ]
    then
        30
...
calculateLargeStraight dice =
    ...
    in
    if
        hasSubsequence sortedValues [ 1, 2, 3, 4, 5 ]
            || hasSubsequence sortedValues [ 2, 3, 4, 5, 6 ]
    then
        40
